    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#080808]/90 backdrop-blur-xl border-b border-white/5">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-1 text-[11px] tracking-[0.24em] uppercase text-[#A8A39A]">
          <button onClick={() => setView("home")} className="px-3 py-2 hover:text-[#F3E7D0] transition">Home</button>
          <button onClick={() => setView("space")} className="px-3 py-2 hover:text-[#F3E7D0] transition">My Space</button>
          <button onClick={() => setView("invite")} className="px-3 py-2 hover:text-[#F3E7D0] transition">Invite</button>
          <button onClick={() => setView("auth")} className="px-3 py-2 hover:text-[#F3E7D0] transition">Log In</button>
          <button
            onClick={() => setView("create-space")}
            className="ml-3 px-5 py-2.5 rounded-full bg-gradient-to-b from-[#D6B76A] to-[#B8893B] text-[#090909] hover:brightness-110 transition"
          >
            Create Space
          </button>
        </nav>
